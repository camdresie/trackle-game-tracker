...
CREATE POLICY "Users can view members of groups they belong to" ON "public"."friend_group_members" FOR SELECT TO "authenticated" USING ("public"."can_user_access_group"("group_id", "auth"."uid"()));
...
CREATE POLICY "Users can view scores of users in the same group" ON "public"."scores" FOR SELECT TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM ("public"."friend_group_members" "my_memberships"
     JOIN "public"."friend_group_members" "their_memberships" ON (("my_memberships"."group_id" = "their_memberships"."group_id")))
  WHERE (("my_memberships"."friend_id" = "auth"."uid"()) AND ("their_memberships"."friend_id" = "scores"."user_id") AND ("my_memberships"."status" = 'accepted'::"text") AND ("their_memberships"."status" = 'accepted'::"text")))) OR (EXISTS ( SELECT 1
   FROM ("public"."friend_groups" "my_groups"
     JOIN "public"."friend_group_members" "their_memberships" ON (("my_groups"."id" = "their_memberships"."group_id")))
  WHERE (("my_groups"."user_id" = "auth"."uid"()) AND ("their_memberships"."friend_id" = "scores"."user_id") AND ("their_memberships"."status" = 'accepted'::"text")))) OR ("auth"."uid"() = "user_id")));
